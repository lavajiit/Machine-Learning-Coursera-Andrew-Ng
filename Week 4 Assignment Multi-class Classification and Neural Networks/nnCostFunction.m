...
for i = 1:m
	...
	y_vec = [1:num_labels]==ones(1,num_labels)*y(i);
	% disp(y_vec);

	J += (y_vec*log(h)+(1-y_vec)*log(1-h));
end
...
% r1 = sum(theta1R.^2);%1*400
r1 = sum(sum(theta1R.^2));
% r2 = sum(theta2R.^2);%1*25
r2 = sum(sum(theta2R.^2));
...
for i = 1:m
	...
	y_vec = [1:num_labels]==ones(1,num_labels)*y(i);
	% disp(y_vec);
	delta_3 = a_3 - y_vec';%10*1

	delta_2 = ((Theta2(:,2:end))'*delta_3).*sigmoidGradient(z_2);%((25*10)*10*1).*25*1 = 25*1
